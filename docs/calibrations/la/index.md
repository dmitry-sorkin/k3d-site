...
* Если у вас хотэнд производительностью примерно до 25 мм³/с, то ускорения и объёмный расход мало влияют на коэффициент PA, и можно не учитывать это влияние. Если вы используете высокопроизводительные хотэнды а-ля Goliath/CHC XL и подобные, то правильнее будет учитывать это влияние и калибровать адаптивный PA;
* На момент составления статьи (февраль 2026 г.) начали появляться принтеры с автоматической калибровкой PA. Но она не всегда точна. Поэтому общая рекомендация - попробовать воспользоваться ей. Если качество печати при этом устроит, то оставить всё как есть. А если при печати будут дефекты, типичные для неправильного коэффициента PA, то переходить на использование ручной калибровки.
...
| OrcaSlicer "шаблон" | + Относительно быстро печатается;<br>+ Тратит мало материала<br>+ Не страдает от проблем с первым слоем | - Скругление углов из-за работы Input Shaping'а приводит к тому, что по этой методике будет получено заниженное значение коэффициента PA;<br>- Нет возможности откалибровать время сглаживания PA;<br>- Нет возможности откалибровать адаптивный PA |
...
| [Методика из документации Klipper](https://www.klipper3d.org/Pressure_Advance.html){ target="_blank" } | + Не страдает от проблем с первым слоем | - Тратит много материала;<br>- Долго печатается;<br>- Скругление углов из-за работы Input Shaping'а приводит к тому, что по этой методике будет получено заниженное значение коэффициента PA;<br>- Нет возможности откалибровать время сглаживания PA;<br>- Нет возможности откалибровать адаптивный PA |
...
2. В левом верхнем углу окна выберите `Калибровка` -> `Прогнозирование расхода`;
...
При относительно небольших ускорениях (до 5000 мм/с^2) стандартное значение сглаживания в 0.04 с не даёт никаких видимых дефектов. Поэтому, если вы не планируете печатать при ускорениях выше, то для твёрдых материалов можно оставить штатное значение.
...
1. В [калибраторе](./calibrator.md){ target="_blank" } установите:
      1. Ускорение = 3000;
      2. Дополнительная цель калибровки = объёмный расход;
      3. Значения дополнительного параметра от 5 мм^3/с до ~90% от максимального объёмного расхода вашего хотэнда;
...
5. Впишите в `измеренные значения адаптивного прогнозирования расхода` через запятую полученное значение PA, расход на задней стенке, 1000. К примеру, если оптимальный коэффициент PA получился 0.05 при расходе 5 мм^3/с, то строка должна выглядеть так: `0.05,5,1000`;
6. Повторите пункты 4 и 5 для других сторон детали, добавляя новые строки в `измеренные значения адаптивного прогнозирования расхода`;
...
Если у вас несколько принтеров, то лучшим решением будет создание отдельных профилей пластика для каждого принтера. Но также возможно и использовать условия. Подробнее об этом можно узнать в [видео про настройки Prusaslicer](https://youtu.be/BMbgFgzPcG8).
...
### Не появляется кнопка "генерировать и скачать"
...
Если после обновления страницы всё равно не появляется кнопка, то, скорее всего, ваш браузер не поддерживает webassembly. В таком случае рекомендуется установить более современный браузер (Chrome, Firefox, Safari, Edge и т. д. последних версий).
...
- На других прошивках эти "проблемные" g-code файлы печатаются также без ошибок и дефектов.
