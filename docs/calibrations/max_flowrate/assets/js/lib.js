...
var firmwareCheckboxes = ["k3d_mfr_marlin", "k3d_mfr_klipper", "k3d_mfr_rrf"];
	...
	switch (key) {
		case "en":
			...
			values["table.bed_size.description"] =
				"[mm] For Cartesian printers - print area size along X/Y axes<br>For Delta printers - bed diameter";
			values["table.firmware.title"] = "Firmware";
			values["table.firmware.description"] =
				"Firmware installed on your printer<br>If you don't know, it's most likely Klipper<br>For Bambu Lab printers, select Marlin";
			values["table.delta.title"] = "Coordinate Origin<br>at Bed Center";
			values["table.delta.description"] =
				"For Cartesian printers should be OFF<br>For Deltas and AD5M should be ON";
			values["table.nozzle_diameter.title"] = "Nozzle Diameter";
			values["table.nozzle_diameter.description"] =
				"[mm] Diameter of the nozzle installed on your printer. Affects line width and layer height when printing samples and helper elements";
			values["table.temperatures.title"] = "Temperatures";
			values["table.temperatures.description"] =
				"[°C] Hotend and bed temperatures when printing the calibration model<br>If the printer has an active chamber, its temperature will be set to 0°C";
			values["table.flowrate.title"] = "Volumetric Flow Rate";
			values["table.flowrate.description"] =
				"[mm^3/s] Initial and final volumetric flow rate value for testing<br>- For V6-like hotends, recommended from 4 to 20<br>- For Volcano-like, recommended from 4 to 30<br>- For Goliath-like, from 4 to 55<br>If a CHT nozzle is installed, consider increasing the maximum test value";
			values["table.z_offset.title"] = "Z-offset";
			values["table.z_offset.description"] =
				"[mm] Vertical offset of the entire model to compensate for incorrect nozzle-to-bed distance on the first layer. Positive value moves the model up, negative - down.<br>If your start G-code already sets a Z-offset, leave it as 0";
			values["table.speed.title"] = "Print Head Movement Speed";
			values["table.speed.description"] =
				"[mm/s] Print head movement speed during printing (print) and travel moves (travel).<br>For print speed, it is recommended to set lower values to minimize the impact of acceleration and deceleration on the average volumetric flow rate during sample printing";
			values["table.flowrate_delta.title"] = "Flow Rate Step";
			values["table.flowrate_delta.description"] =
				"[mm^3/s] Difference in volumetric flow rate between adjacent test elements. The smaller the step, the more samples will be printed";
			values["table.extrusion_length.title"] = "Extrusion Length<br>per Sample";
			values["table.extrusion_length.description"] =
				"[mm] The length of filament to be extruded for printing each sample. Measured at the input to the feed mechanism";
			values["table.start_gcode.title"] = "Start G-code";
			values["table.gcodes.description"] =
				"For printing to be correctly initialized and finalized, you need to provide G-codes suitable for your printer. The easiest way is to copy them from your slicer. If the slicer's G-codes do not contain unsupported elements, you can copy them unchanged. If there are unsupported elements, you will have to remove them - manually replace unsupported placeholders with values, calculate results of mathematical functions, expand conditions, etc.<hr><u>Supported placeholders:</u><br>- <code>$HOTTEMP</code>, <code>{temperature}</code>, <code>{first_layer_temperature}</code>, <code>{nozzle_temperature}</code> and similar will be replaced with the hotend temperature;<br>- <code>$BEDTEMP</code>, <code>{bed_temperature}</code>, <code>{first_layer_bed_temperature}</code>, <code>{hot_plate_temp}</code>, <code>{textured_plate_temp_initial_layer}</code> and similar will be replaced with the bed temperature;<br>- <code>{chamber_temperature}</code>, <code>{chamber_minimal_temperature}</code> will be replaced with 0;<br>- Various modifications of the placeholders mentioned above. For example, using square brackets instead of curly braces, specifying an array element index, arithmetic inside curly braces, etc.<hr><u>Not supported:</u><br>- Other placeholders;<br>- Conditional statements (<code>if</code>);<br>- Ternary operators (<code>a ? b : c</code>);<br>- Mathematical functions and expressions";
			...
			values["error.bed_size_x.format"] =
				"Format Error: Print Area Size along X axis";
			values["error.bed_size_x.value"] =
				"Value Error: Print Area Size along X axis (less than 100 or greater than 1000 mm)";
			values["error.bed_size_y.format"] =
				"Format Error: Print Area Size along Y axis";
			values["error.bed_size_y.value"] =
				"Value Error: Print Area Size along Y axis (less than 100 or greater than 1000 mm)";
			values["error.nozzle_diameter.format"] = "Format Error: Nozzle Diameter";
			values["error.nozzle_diameter.value"] =
				"Value Error: Nozzle Diameter (less than 0.1 or greater than 2.0 mm)";
			values["error.hotend_temp.format"] = "Format Error: Hotend Temperature";
			values["error.hotend_temp.value"] =
				"Value Error: Hotend Temperature (less than 100°C or greater than 500°C)";
			values["error.hotend_temp.too_low"] =
				"Value Error: Hotend Temperature (less than 100°C)";
			values["error.hotend_temp.too_high"] =
				"Value Error: Hotend Temperature (greater than 500°C)";
			values["error.bed_temp.format"] = "Format Error: Bed Temperature";
			values["error.bed_temp.value"] =
				"Value Error: Bed Temperature (greater than 150°C)";
			values["error.bed_temp.too_low"] =
				"Value Error: Bed Temperature (less than 0°C)";
			values["error.bed_temp.too_high"] =
				"Value Error: Bed Temperature (greater than 150°C)";
			values["error.init_flowrate.format"] =
				"Format Error: Initial Volumetric Flow Rate value";
			values["error.init_flowrate.value"] =
				"Value Error: Initial Volumetric Flow Rate value (less than 1.0 or greater than 50.0 mm^3/s)";
			values["error.end_flowrate.format"] =
				"Format Error: Final Volumetric Flow Rate value";
			values["error.end_flowrate.value"] =
				"Value Error: Final Volumetric Flow Rate value (less than 1.0 or greater than 200.0 mm^3/s)";
			values["error.flowrate_delta.format"] = "Format Error: Flow Rate Step";
			values["error.flowrate_delta.value"] =
				"Value Error: Flow Rate Step (less than 0.01 or greater than 10.0)";
			values["error.extrusion_length.format"] =
				"Format Error: Extrusion Length per Sample";
			values["error.extrusion_length.value"] =
				"Value Error: Extrusion Length per Sample (less than 50 or greater than 5000 mm)";
			values["error.z_offset.format"] = "Format Error: Z-offset";
			values["error.z_offset.value"] =
				"Value Error: Z-offset (less than -0.5 or greater than 0.5 mm)";
			values["error.speed.format"] =
				"Format Error: Print Head Movement Speed during sample printing";
			values["error.speed.value"] =
				"Value Error: Print Head Movement Speed during sample printing (less than 5 or greater than 500 mm/s)";
			values["error.travel_speed.format"] =
				"Format Error: Print Head Movement Speed during travel moves";
			values["error.travel_speed.value"] =
				"Value Error: Print Head Movement Speed during travel moves (less than 5 or greater than 1000 mm/s)";
			...
			values["error.unsupported_math"] =
				"Error: Mathematical operations (min, max, round, etc.) detected in Start or End G-code";
			values["error.different_versions"] =
				"Version mismatch: Please hard refresh (Ctrl + F5) or clear your browser cache.";

			values["info.out_of_print_area"] =
				"Some samples did not fit on the bed. Number of skipped samples: %s. Increase the test value step or reduce the speed\n";
			values["info.cali_params_template"] =
				";Sample: %d Flow Rate: %.3f mm^3/s\n";
		...
		case "ru":
			...
			values["table.nozzle_diameter.description"] =
				"[мм] Диаметр сопла, установленного на вашем принтере. Влияет на ширину линии и толщину слоя при печати образцов и вспомогательных элементов";
			...
			values["table.flowrate.description"] =
				"[мм^3/с] Начальное и конечное значение объёмного расхода для проверки<br>- Для V6-подобных хотэндов рекомендуется от 4 до 20<br>- Для Volcano-подобных рекомендуется от 4 до 30<br>- Для Goliath-подобных от 4 до 55<br>При наличии CHT вставки в хотэнде, стоит увеличить максимальное проверяемое значение";
			values["table.z_offset.title"] = "Z-offset";
			values["table.z_offset.description"] =
			...
			values["table.speed.description"] =
				"[мм/с] Скорость движения печатающей головы при печати (print) и при холостых перемещениях (travel).<br>Для скорости печати рекомендуется ставить невысокие значения, чтобы разгон и торможение оказывали минимальное влияние на средний объёмный расход при печати образца";
			values["table.flowrate_delta.title"] = "Шаг расхода";
			values["table.flowrate_delta.description"] =
				"[мм^3/с] Разница объёмного расхода между соседними проверяемыми элементами. Чем меньше, тем больше образцов будет напечатано";
			values["table.extrusion_length.title"] = "Длина экструзии<br>на образец";
			values["table.extrusion_length.description"] =
				"[мм] Сколько мм прутка будет экструдировано для печати каждого образца. Считается по входу в подающий механизм";
			...
			values["error.hotend_temp.value"] =
				"Ошибка значения: Температура хотэнда (меньше 100°C или больше 500°C)";
			values["error.hotend_temp.too_low"] =
				"Ошибка значения: Температура хотэнда (меньше 100°C)";
			values["error.hotend_temp.too_high"] =
				"Ошибка значения: Температура хотэнда (больше 500°C)";
			values["error.bed_temp.format"] = "Ошибка формата: Температура стола";
			values["error.bed_temp.value"] =
				"Ошибка значения: Температура стола (выше 150°C)";
			values["error.bed_temp.too_low"] =
				"Ошибка значения: Температура стола (меньше 0°C)";
			values["error.bed_temp.too_high"] =
				"Ошибка значения: Температура стола (выше 150°C)";
			values["error.init_flowrate.format"] =
				"Ошибка формата: начальное значение объёмного расхода";
			values["error.init_flowrate.value"] =
				"Ошибка значения: начальное значение объёмного расхода (меньше 1.0 или больше 50.0 мм^3/с)";
			values["error.end_flowrate.format"] =
				"Ошибка формата: конечное значение объёмного расхода";
			values["error.end_flowrate.value"] =
				"Ошибка значения: конечное значение объёмного расхода (меньше 1.0 или больше 200.0 мм^3/с)";
			values["error.flowrate_delta.format"] = "Ошибка формата: Шаг расхода";
			values["error.flowrate_delta.value"] =
				"Ошибка значения: Шаг расхода (меньше 0.01 или больше 10.0)";
			values["error.extrusion_length.format"] =
				"Ошибка формата: длина экструзии на образец";
			values["error.extrusion_length.value"] =
				"Ошибка значения: длина экструзии на образец (меньше 50 или больше 5000 мм)";
			values["error.z_offset.format"] = "Ошибка формата: Z-offset";
			values["error.z_offset.value"] =
				"Ошибка значения: Z-offset (меньше -0.5 или больше 0.5 мм)";
			values["error.speed.format"] =
				"Ошибка формата: скорость движения печатающей головы при печати образцов";
			values["error.speed.value"] =
				"Ошибка значения: скорость движения печатающей головы при печати образцов (меньше 5 или больше 500 мм/с)";
			values["error.travel_speed.format"] =
				"Ошибка формата: скорость движения печатающей головы при холостых перемещениях";
			values["error.travel_speed.value"] =
				"Ошибка значения: скорость движения печатающей головы при холостых перемещениях (меньше 5 или больше 1000 мм/с)";
			...
			values["info.out_of_print_area"] =
				"Некоторые образцы не поместились на стол. Количество пропущенных образцов: %s. Увеличьте шаг проверяемых значений или снизьте скорость\n";
			values["info.cali_params_template"] =
				";Образец: %d Расход: %.3f мм^3/с\n";
			break;
	}
...
function initProfileManager() {
	...
	profileManager = new ProfileManager("k3d_mfr_profiles", 10);
...
function exportProfile() {
	...
	if (profiles.length === 0) {
		...
		downloadJSON("mfr_calibrator_profiles.json", json);
		...
		downloadJSON(`mfr_calibrator_profiles_${timestamp}.json`, json);
