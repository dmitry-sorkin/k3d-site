...
1. В разделе `[printer]`:
      ...
      2. Установить `max_accel_to_decel` равным ускорению, установленному в предыдущем пункте ИЛИ установить `minimum_cruise_ratio: 0`;
         ...
         ![](./pics/decel_vs_cruise_compressed.png)  
...
2. Откройте её в любом слайсере и нарежьте со следующими параметрами:
      ...
      6. Количество периметров значения не имеет. Можно выставить в 1;
      7. Перед экспортом файла рекомендуется посмотреть на диаграмму скоростей печати и убедиться, что скорости действительно максимальные. Если это не так, проверьте настройки слайсера. Возможно включено замедление печати при слишком быстрой печати одного слоя.
...
После этого запустите печать ранее нарезанного G-кода и дождитесь её завершения. На получившейся модели каждый сегмент печатается с разной частотой шейпера на ось Y. На нижнем сегменте частота равна таковой из аргумента `START` в предыдущей команде. Каждый следующий сегмент печатается с частотой на `STEP_DELTA` больше, чем предыдущий.
...
После этого запустите печать ранее нарезанного G-кода и дождитесь её завершения. На получившейся модели на оси Y действует шейпер `mzv` с частотой, подобранной ранее. На оси X частота шейпера меняется начиная со значения аргумента `START` с шагом `STEP_DELTA`.
...
4. Рассчитайте максимальные ускорения, как это описано в разделе "подбор максимальных ускорений" ниже;
5. Распечатайте модель с полученными шейперами и ускорениями. Если качество печати вас устраивает, то подбор частот закончен.
