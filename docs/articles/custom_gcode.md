...
Если вы купили себе готовый принтер, то лучшим вариантом для начала будет открыть готовый профиль под этот принтер в том слайсере, в котором вы собираетесь дальше работать. При этом важно отметить, что G-коды из PrusaSlicer и OrcaSlicer совместимы, если там не используются плейсхолдеры для каких-то параметров, которых в другом слайсере нет. То есть в большинстве случаев можно скопировать начальный/конечный G-код из OrcaSlicer в PrusaSlicer и наоборот.
...
G1 E-5 F1800 #Сделать откат 5 мм
...
Если нет, то придётся искать готовый профиль, в котором кто-то другой уже написал алгоритм для использования инструментов очистки сопла, или писать и отлаживать алгоритм самостоятельно.
