...
4. После записи в корне MicroSD появится файл `system.cfg`. Откройте его любым текстовым редактором, например, [Notepad++](https://notepad-plus-plus.org/downloads/){ target="_blank" };
5. В нём укажите:
      1. `check_interval=5`;
      2. `eth=end0`;
      3. `wlan=wlan0`;
...
!!! note "Далее все инструкции будут даваться для подключения из-под пользователя (biqu). При выполнении команд через sudo надо будет вводить пароль `root`"
...
В VOSTOK в качестве коммутационных плат печатающих голов используются Fysetc H36. Они могут подключаться как по USB, так и по CAN. Но, в случае подключения по USB, теплостойкость этих плат снижается. Поэтому используется подключение по CAN. Для этого необходимо прошить Octopus Pro v1.1 в режим USB-to-CAN Bridge.

Есть несколько способов прошить Octopus под этот конфиг. Но лучше всего заменить загрузчик на Katapult т.к. это позволит поднимать пин ws7040 сразу после включения платы, убирая несколько секунд работы системы охлаждения на полных оборотах. Также Katapult упрощает обновление прошивки в будущем.

### Установка Katapult

1. Подключите к хосту только Octopus Pro. Fysetc H36 не подключайте никуда;
2. Подключитесь к хосту по SSH;
3. Откройте меню сборки Katapult:
...
4. Установите значение настроек следующим образом:
    ...
    - GPIO pins to set on bootloader entry: `PD12`;
...
8. Убедитесь, что плата видна в системе как устройство DFU:
    ...
    В выводе должна появиться строка вида `STMicroelectronics STM Device in DFU mode`. Если её нет — повторите шаг 7;

9. Прошейте Katapult в Octopus:

    ```bash
    sudo dfu-util -R -a 0 -s 0x08000000:mass-erase:force:leave -D ~/katapult/out/katapult.bin -d 0483:df11
    ```

    Если в выводе появилась строка `File downloaded successfully`, но выше неё выскочила ошибка `Error during download get_status` — прошивка прошла успешно, ошибку можно игнорировать

    !!! info "Если вы видите другую ошибку, и не видите `File downloaded successfully`, то попробуйте:"

        ``` bash
        sudo dfu-util -R -a 0 -s 0x08000000:leave -D ~/katapult/out/katapult.bin -d 0483:df11
        ```

10. Убедитесь, что Katapult виден хосту:

    ``` bash
    ls /dev/serial/by-id
    ```

    Вы должны увидеть строку вида `usb-katapult_*****************`. Это значит, что устройство прошилось и загрузилось в Katapult.

    !!! info "Если вы видите `usb-klipper_**************`, то дважды быстро нажмите `RESET` на материнской плате. Это переведёт её в Katapult"

    !!! warning "Если список устройств пуст, то не продолжайте. Попробуйте прошить Katapult еще раз"
...
2. Откройте меню сборки Klipper:
...
3. Установите значение настроек следующим образом:
    ...
    - GPIO pins to set at micro-controller setup: `PD12`;
...
6. Если после прошивки Katapult вы перезагружали плату, то снова войдите в Katapult дважды быстро нажав `RESET` на Octopus Pro;
7. Убедитесь, что Katapult виден хосту:
...
8. Прошейте Klipper через Katapult по USB, подставив путь из предыдущего шага:

    ```bash
    python3 ~/katapult/scripts/flashtool.py -f ~/klipper/out/klipper.bin -d /dev/serial/by-id/usb-katapult_*******
    ```

    !!! warning "Не забудьте изменить `*******` на название вашей платы, полученное из `ls /dev/serial/by-id`"

9. Дождитесь окончания прошивки. После этого плата автоматически перезагрузится уже в Klipper;
10. Включите сервис Klipper обратно:

    ``` bash
    sudo service klipper start
    ```

11. Проверьте, что Octopus отображается как USB-to-CAN адаптер:

    ```bash
    lsusb
    ```

    В списке должна появиться строка с упоминанием `Geschwister Schneider CAN adapter` (или аналогичный USB-to-CAN мост). Если строка есть — прошивка прошла успешно.

!!! info "После прошивки Octopus будет виден в системе и как USB-to-CAN адаптер (для работы сети CAN), и как serial-устройство. Не путайте их при последующих перепрошивках"
...
3. Установите значение настроек следующим образом:
    ...
    - CAN bus speed: `1000000`;
    - GPIO pin to set at micro-controller startup: `!PA2`;
    - Support bootloader entry on rapid double click of reset button: `Вкл.`;
    - Enable bootloader entry on button (or gpio) state: `Выкл.`;
    - Enable Status LED: `Вкл.`;
