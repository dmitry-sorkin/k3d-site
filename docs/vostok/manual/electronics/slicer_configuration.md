...
!!! note "Чтобы скачать, надо быть залогиненым на GitHub"
...
- Температуры работают корректно. То есть каждый хотэнд будет нагрет до той температуры, которая указана в его профиле филамента. Переключение с температуры первого слоя на температуру остальных слоёв тоже работает индивидуально для каждого хотэнда;
- Температура стола будет выставлена по максимальной температуре стола для двух материалов;
- Поток, настройки откатов, ограничение максимального объёмного расхода и т.д. применяются только от левой печатающей головы;
