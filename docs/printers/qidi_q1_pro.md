...
5. Модуль координаты по Z, указанной в веб-интерфейсе, является вашим Z-offset. Впишите это значение в конфигурацию в раздел `probe` в параметр `z_offset`. К примеру, если координата `-1.16`, то в конфигурацию надо вписать `z_offset: 1.16`;
6. Сохраните изменения и перезагрузите прошивку.
