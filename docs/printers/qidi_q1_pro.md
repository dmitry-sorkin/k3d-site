...
В дополнение к этому у принтера отличная электроника, не кастрированная ни в чём, с относительно чистой и хорошо настроенной прошивкой и очень функциональным мобильным приложением. А также хорошая комплектация - веб-камера, подсветка, датчик наличия филамента и т.д.
...
4. Посмотрите как наклонён стол и постарайтесь поправить это вращением барашков под столом. 1 полный оборот = перемещение точки стола над винтом на 0.5 мм. Затягивание барашка опускает точку, откручивание барашка поднимает точку;
5. Повторяйте шаги 2-4 пока получается уменьшать перепад высот стола. Если удалось добиться перепада высот менее 0.2 мм, то больше ничего делать не нужно. Если такого перепада высот добиться не удаётся, то остановитесь на максимально хорошем результате, которого удалось добиться, и переходите к следующему шагу.
...
6. Повторяете пункты 2-5 пока кривизна стола не станет менее 0.15 мм. Важно не пропускать никакие действия т. к. иначе снятая карта высот не будет соответствовать таковой при реальной печати.
...
!!! warning "Практика использования MicroProbe в активной термокамере показала, что у некоторых они работают долго и без проблем, а у других могут довольно быстро выйти из строя. Связано это с конструкцией самого датчика. Поэтому рекомендуется рассмотреть возможность оставить штатный датчик автоуровня или установить Klicky Probe или подобный ему датчик"
...
- Парковка и автоуровень становятся очень стабильными. Больше нет, хоть и редких, но всё же иногда возникающих ситуаций, когда сопло плохо почистилось, из-за этого неправильно автоматически выставился Z-offset датчика, из-за чего плохо выложился первый слой и печать надо начинать заново. С MicroProbe надо только настроить 1 раз Z-offset вручную и дальше каждая печать начинается при одинаковом расстоянии от сопла до стола;
...
| Сопло Volcano | [:material-link:](../part-navi/extruder.md){ target="_blank" } | Сопло выбирайте на свой вкус. Лично я печатаю через стальное закалённое CHT 0.4 и 0.6 мм. У вас могут быть другие предпочтения |
...
Если вам очень важен уровень шума и вы готовы терпеть небольшие искажения геометрии, то вы можете перевести драйверы обратно в StealthChop. Для этого в секциях `tmc2240 stepper_x` и `tmc2240 stepper_y` измените значение параметра `stealthchop_threshold` на 999999.
...
Корпус системы охлаждения отличается от штатного двумя вещами: креплением вентилятора и магнитами. Магниты Ø5x1 приклеиваются на выступы под них. Рекомендую предварительно проверить, что полюса не перепутаны.
...
4. С помощью инструментов движения, поднимайте стол по 0.1 мм пока сопло не притрёт бумажку;
