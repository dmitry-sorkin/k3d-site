...
### 2. Отказ от автоматического выставления рамы стола

Также важно не выполнять команду `Z_TILT_ADJUST` после калибровки стола т.к. это приведёт к изменению положения рамы и, следовательно, положения стола. В свою очередь, это приведёт к необходимости заново калибровать стол. Чтобы такого не происходило, надо убрать все вызовы `Z_TILT_ADJUST` из макросов:
...
- Новый датчик автоуровня сразу устанавливается в такую позицию, что становится возможной установка сопел Volcano вместо стандартных. Эти сопла на 2мм (~10%) длиннее, благодаря чему немного вырастет производительность хотэнда. Кроме того, Volcano сопла более распространены, чем стоковые, существует большее количество вариаций этих сопел. Некоторые из них, например, CHT варианты, лучше прогревают центр филамента, из-за чего, теоретически, может увеличиться прочность деталей из плохо спекающихся материалов;
...
|:------------ |:---------- |
| [Крепление MicroProbe](./models/k3d_q1_microprobe_mount.stl){ download="k3d_q1_microprobe_mount.stl"} | Печатать необходимо из жёсткого и стойкого к температуре материала. Самым простым из таковых будет ABS. Предпочтительно использовать сопло 0.4 мм т.к. деталь небольшая |
| [Корпус системы охлаждения под Volcano сопла](./models/k3d_q1_volcano_cooling_system_adaptation.stl.stl){ download="k3d_q1_volcano_cooling_system_adaptation.stl.stl" } | Необязательно. Корпус системы охлаждения перепечатывается для того, чтобы перенести систему охлаждения ниже на 2мм (на столько увеличилась длина хотэнда). Если не переносить, то всё соберётся и принтер будет работать нормально. Просто эффективность охлаждения, скорее всего, немного пострадает |
...
1. [:material-download: Скачайте конфигурацию](./files/printer.cfg){ download="printer.cfg" };
...
!!! warning "После установки конфигурации необходимо будет откалибровать Z-offset датчика (`z_offset` в разделе `probe`). Также, возможно, придётся поменять позицию при парковке (`position_endstop` в разделе `stepper_z`)"
...
Сопло Volcano устанавливается вместо стокового обычным методом. Использовать модификацию без замены сопла невозможно т.к. со стоковым соплом датчик окажется ниже сопла и печатать будет нельзя.
...
1. Выполните автопарковку `G28`;
2. Сдвиньте печатающую голову примерно в центр стола `G1 X125 Y125`;
3. С помощью инструментов движения, поднимайте стол по 0.1мм пока сопло не притрёт бумажку;
4. Координата по Z, указанная в веб-интерфейсе, является вашим Z-offset. Впишите её в конфигурацию в раздел `probe` в параметр `z_offset`;
5. Сохраните изменения и перезагрузите прошивку.
