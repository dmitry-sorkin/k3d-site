...
Скопируйте эти макросы в конец файла gcode_macro.cfg, после чего сохранитесь и перезагрузите прошивку.
...
ssh mks@ip_адрес_принтера
```
IP можно посмотреть прямо через экран принтера, он, обычно, имеет вид `192.168.xxx.xxx`. Пароль `makerbase`;
