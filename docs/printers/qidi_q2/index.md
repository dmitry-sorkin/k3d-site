...
1. В веб-интерфейсе принтера зайдите во вкладку `Конфигурация` и откройте файл `printer.cfg`;
2. Раскомментируйте (уберите `#` в начале строки) строку `[include KAMP_Settings.cfg]`;
3. Сохраните изменения и перезагрузите прошивку.

Также можно включить адаптивную линию прочистки, которая точно не будет пересекаться с печатаемой деталью:

1. В веб-интерфейсе принтера зайдите во вкладку `Конфигурация` и откройте файл `KAMP_Settings.cfg`;
2. В нём раскомментируйте `[include ./KAMP/Line_Purge.cfg]`;
