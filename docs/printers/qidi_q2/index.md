...
Иными словами, если ваши задачи ограничиваются базовой печатью из простых материалов (PLA, PETG и т.п.), то лучше присмотреть другую, менее косячную модель. Если же в вашей практике часто встречаются задачи инженерной печати, задачи печати эластомерами, или вам просто хочется наиболее универсальный принтер, то придётся брать Q2 т. к. других моделей с таким же набором функций за сходные деньги на данный момент не известно.
...
!!! example "Работа рекомендаций из статьи была проверена на прошивках 1.0.6-1.1.1. Работа на других версиях прошивки не гарантируется"
...
## Настройка пользователя qidi

!!! note "На старых прошивках пользователь `mks` работает нормально, и настройка пользователя `qidi` не нужна. Если вы пользуетесь старой прошивкой, то в дальнейших инструкциях выполняйте всё из-под пользователя `mks`, а не `qidi`"

В прошивке `01.01.02.03` домашняя директория пользователя `mks` является симлинком на домашнюю директорию `qidi`. Поэтому из-под `mks` нельзя записывать файлы в домашнюю директорию. Перед выполнением остальных инструкций один раз настройте пользователя `qidi`:

<div class="steps" markdown>

1. Подключитесь к принтеру по SSH под пользователем `mks`:

    ```
    ssh mks@ip_адрес_принтера
    ```

    !!! info "Пароль по умолчанию — `makerbase`"

2. Измените пароль пользователя `qidi`:

    ```
    sudo passwd qidi
    ```

    Рекомендуется указать пароль `makerbase` чтобы потом не забыть его;

3. Добавьте `qidi` в группу `sudo`:

    ```
    sudo usermod -aG sudo qidi
    ```

4. Завершите SSH-сеанс <kbd>Сtrl + C</kbd> и подключитесь к принтеру уже под пользователем `qidi`:

    ```
    ssh qidi@ip_адрес_принтера
    ```
...
Первая - для штатного акселерометра сильно занижена частота дискретизации (400 Гц вместо 1600 Гц), из-за чего тот снимает показания неправильно, с большим количеством артефактов. Из-за них автокалибровщик не может подобрать оптимальные шейперы, и рекомендует неоптимальные, при которых остаётся большое количество эхо.
...
Если небольшое эхо на деталях останется, то уменьшите частоты до 75 или 70 Гц. Максимальные ускорения при этом составят ~6300 и ~5500 мм/с^2.
...
2. Подключитесь к принтеру по SSH под пользователем `qidi`:

    ```
    ssh qidi@ip_адрес_принтера
    ```

    !!! note "IP можно посмотреть прямо через экран принтера, он, обычно, имеет вид `192.168.xxx.xxx`"

3. Введите пароль пользователя `qidi`. Символы при этом не отображаются, но знаки вводятся;
4. Введите команду
...
5. Со всем соглашайтесь и ждите окончания установки. Если спросит пароль администратора, введите пароль пользователя `qidi`, заданный при подготовке прошивки. После окончания установки дополнительных действий не требуется.
...
1. Подключитесь к принтеру по SSH `ssh qidi@ip_адрес_принтера`, используя пароль пользователя `qidi`;
...
4. Зайдите в веб-интерфейс принтера, в раздел `Конфигурация`. Там должна появиться папка `shapers`. В ней вы найдёте сгенерированные амплитудно-частотные характеристики. На них в правом-верхнем углу будут указаны подобранные автоподборщиком шейперы. Жирным выделен рекомендуемый шейпер;
5. Перейдите в раздел `Конфигурация` и откройте `printer.cfg`;
6. Промотайте в конец файла. Там будет много серых строк, начинающихся с `#*#`. Среди них найдите раздел `[input_shaper]`, который должен выглядеть примерно так:
...
7. Сохраните и перезагрузитесь

</div>

## Адаптивная карта высот стола

!!! info "В новой прошивке KAMP включен по умолчанию. Следующая инструкция нужна только для старых прошивок, в которых KAMP отключен"

Адаптивная карта высот стола позволяет снимать карту высот только в тех зонах, где будет идти печать. Это позволяет тратить меньше времени на подготовку к печати, а также улучшает первый слой.

В старых прошивках KAMP можно включить следующим образом:
...
4. Откройте файл `KAMP_Settings.cfg` в папке `KAMP`;
5. Добавьте строку в начало файла:

    ```
    [include Adaptive_Meshing.cfg]
    ```

6. Убедитесь, что строка `#[include ./KAMP/Adaptive_Meshing.cfg]` закомментирована. Если нет, то закомментируйте;
...
1. В веб-интерфейсе принтера зайдите во вкладку `Конфигурация`, и откройте файл `KAMP/KAMP_Settings.cfg`;
2. Добавьте строку в начало файла:

    ```
    [include Line_Purge.cfg]
    ```

3. Убедитесь что строка `#[include ./KAMP/Line_Purge.cfg]` закомментирована. Если нет, то закомментируйте;
4. Установите значение параметра `variable_mesh_margin` на 10;
5. Установите значение параметра `variable_fuzz_amount` на 2;
6. Нажмите кнопку `Сохранить и перезапустить`.
...
!!! warning "Используя эти макросы, вы полностью берёте на себя ВСЕ риски"
!!! warning "Эти макросы не подходят к принтерам с QIDI Box"
!!! note "Если вы использовали эти макросы, и они вам понравились, или у вас есть какие-то претензии, то напишите об этом в [телеграм-чат K3D](https://t.me/K_3_D){ target="_blank" }. Обратная связь поможет найти и устранить проблемы"

### Макросы

Скопируйте эти макросы в конец файла `klipper-macros-qd/gcode_macro.cfg`, после чего сохранитесь и перезагрузите прошивку.

В старых версиях прошивки этот файл мог называться просто `gcode_macro.cfg`. В новых версиях макросы находятся в папке `klipper-macros-qd`.
...
!!! warning "Если вы используете старую прошивку, в которой KAMP не включен, удалите из макроса `POEHALI` строку `LINE_PURGE`. Прочистку сопла в таком случае выполняйте с помощью печати юбки"
...
1. Откройте файл `klipper-macros-qd/gcode_macro.cfg`;
...
2. Подключитесь к принтеру по SSH. Для этого введите команду:
    ...
    IP можно посмотреть прямо через экран принтера, он, обычно, имеет вид `192.168.xxx.xxx`. Используйте пароль пользователя `qidi`;
